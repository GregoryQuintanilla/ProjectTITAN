...
function calcRoute(origin, stops, directionsService, directionsRenderer){
     ...
     distanceMatrix.getDistanceMatrix(
     {
               ...
               destinations: stops,
               travelMode: 'DRIVING',
     },function(response,status){chooseNextDestination(response, status, directionsService, directionsRenderer )}); // create the distance matrix and then call the chooseNextDestination callback function
     return route //eventually return the completed route
}

function chooseNextDestination(response, status, directionsService, directionsRenderer){
     if (status =='OK'){ //if the callback was successful
          ...
          if(destinations.length != 0){
               distanceMatrix.getDistanceMatrix(
               {
                    ...
                    travelMode: 'DRIVING',
               },function(response, status) {chooseNextDestination(response, status, directionsService, directionsRenderer )}} );
          }
...
function displayRoute(routeToDisplay,directionsService,directionsRenderer){
